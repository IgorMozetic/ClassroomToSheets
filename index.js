function importClassroomToSheets() {
  ...
  if (alunos){
    ...
    alunos.forEach((aluno)=>{
      let sheetData = [data.toLocaleDateString("pt-BR"),curso.id, curso.name, aluno.id, aluno.nome, aluno.email, curso.courseState]
...
function getCourses(){
  ...
  return listaClassroomCompleta = listaClassroomCompleta.filter((classroom) => {
    if(classroom.ownerId === "107558995070093037568"){
